...
install() {
    bash <(curl -Ls https://github.com/875706361/875706361/raw/master/v2-ui-master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/875706361/875706361/master/v2-ui-master/v2-ui.sh)
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://raw.githubusercontent.com/875706361/875706361/master/v2-ui-master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
